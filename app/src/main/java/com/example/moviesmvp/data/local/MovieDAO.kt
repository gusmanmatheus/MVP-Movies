...
class MovieDAO( context: Context) :BaseDAO(context) {
    ...
    fun getMovies(page:Int):List<Movie>{
        ...
        val cursor = db.query(MOVIE_DATA.TABLE_NAME,columns,"${MOVIE_DATA.PAGE} =?", arrayOf("$page"),null, null,null)
    ...
    fun deleteMovie(idMovie: Int){
        val db = writableDatabase
         db.delete(MOVIE_DATA.TABLE_NAME,"${MOVIE_DATA.ID_MOVIE} = $idMovie",null)
        db.close()
    ...
    fun getMovie(idMovie: Int):List<Int>{
        ...
        val cursor = db.query(MOVIE_DATA.TABLE_NAME, arrayOf(MOVIE_DATA.ID_MOVIE),"${MOVIE_DATA.ID_MOVIE} =?",
            arrayOf(" $idMovie"),null,null,null)
        if( cursor != null && cursor.moveToFirst() ){
            do{
